        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if state == "START_SCREEN":
                    button_rect = self.gui.draw_start_screen()
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if button_rect.collidepoint(event.pos):
                            state = "PLAYING"
                            lives = 3
                            self.game_state.reset()  # Gọi hàm reset trạng thái game
            ...
            if state == "START_SCREEN":
                self.gui.draw_start_screen()
            elif state == "PLAYING":
                self.gui.draw_screens()
                self.all_sprites.draw(self.screen)
                self.all_sprites.update(dt)
                # Kiểm tra Pacman chết
                if self.game_state.is_pacman_dead:
                    lives -= 1
                    if lives <= 0:
                        state = "START_SCREEN"
                    else:
                        self.gui.pacman_dead_reset()
